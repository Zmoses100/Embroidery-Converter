...
class ConversionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (! $user->canConvert()) {
            return back()->withErrors([
                'limit' => 'You have reached your daily conversion limit. Please upgrade your plan.',
            ]);
        }
        ...
        // Local ownership authorization fix
        if ((int) $sourceFile->user_id !== (int) $user->id) {
            abort(403, 'You are not allowed to convert this file.');
        }
    ...
    public function batch(Request $request)
    {
        ...
        foreach ($request->file_ids as $fileId) {
            ...
            if (! $sourceFile || (int) $sourceFile->user_id !== (int) $user->id) {
                continue;
            }
        ...
        if ($dispatched === 0) {
            return back()->withErrors([
                'batch' => 'No conversions were started. Check your daily limit.',
            ]);
        }
    ...
    public function index(Request $request)
    {
        ...
        $conversions = $user->conversions()
            ...
            ->when($request->format, fn ($q, $f) => $q->where('target_format', strtolower($f)))
            ->latest()
            ->paginate(20)
            ->withQueryString();
    ...
    public function show(Conversion $conversion)
    {
        ...
        if (! $user || (int) $conversion->user_id !== (int) $user->id) {
            abort(403, 'You are not allowed to view this conversion.');
        }
    ...
    public function status(Conversion $conversion)
    {
        ...
        return response()->json([
            ...
            'download' => $conversion->outputFile
                ? route('files.download', $conversion->output_file_id)
                : null,
        ]);
    }
}