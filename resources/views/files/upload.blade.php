...
<div class="max-w-3xl mx-auto space-y-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">
            Supported formats: {{ implode(', ', array_map('strtoupper', $supportedFormats)) }}
        </p>
    </div>
    ...
    <form method="POST" action="{{ route('files.store') }}" enctype="multipart/form-data" id="upload-form">
        ...
        <div x-data="{
            ...
            <div :class="dragging ? 'border-primary-500 bg-primary-50' : 'border-gray-300 bg-white hover:border-primary-400'"
                ...
                <p class="text-sm text-gray-400 mt-1">
                    Max 20 files · Supported: {{ implode(', ', array_map('strtoupper', array_slice($supportedFormats, 0, 8))) }}...
                </p>
                <input id="file-input" type="file" name="files[]" multiple x-ref="fileInput"
                       accept="{{ implode(',', array_map(fn($f) => '.' . $f, $supportedFormats)) }}"
        ...
        <div class="flex items-center justify-between mt-4">
            <a href="{{ route('files.index') }}" class="text-sm text-gray-500 hover:text-gray-700">← Back to library</a>
            <button type="submit"
