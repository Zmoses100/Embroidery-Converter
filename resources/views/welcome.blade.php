...
<head>
    ...
    <title>{{ config('app.name', 'Embroidery Converter') }} - Convert Embroidery Files Online</title>
...
<header class="fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur border-b border-gray-100">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-6 text-sm font-medium text-gray-600">
            ...
            <a href="{{ route('plans.index') }}" class="hover:text-gray-900">Pricing</a>
        </nav>
        <div class="flex items-center gap-3">
            @auth
                <a href="{{ route('dashboard') }}" class="text-sm font-medium text-primary-600 hover:text-primary-700">Dashboard →</a>
            @else
...
<section class="pt-28 pb-20 bg-gradient-to-br from-primary-50 via-white to-indigo-50">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 text-center">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <a href="{{ route('plans.index') }}" class="px-8 py-4 bg-white text-gray-700 text-base font-semibold rounded-xl border border-gray-200 hover:border-gray-300 hover:bg-gray-50 transition-all">
                View Plans
            </a>
        </div>
...
<section id="formats" class="py-16 bg-white">
    <div class="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div class="flex flex-wrap justify-center gap-3">
            @foreach(['PES','DST','JEF','EXP','VP3','HUS','XXX','SEW','VIP','PEC','PCS','SHV','CSV','DAT','DSB','DSZ','FXY','KSM','PCD','PCQ','RGB','STC','STX','TAP','U01','ZHS','ZXY'] as $fmt)
                <span class="px-3 py-1.5 bg-primary-50 text-primary-700 text-sm font-bold rounded-lg border border-primary-100">{{ $fmt }}</span>
            @endforeach
        </div>
    </div>
</section>
...
<footer class="bg-gray-900 text-gray-400 py-10">
    <div class="max-w-5xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4 text-sm">
        <p>© {{ date('Y') }} Embroidery Converter. All rights reserved.</p>
        <div class="flex gap-6">
            <a href="{{ route('plans.index') }}" class="hover:text-white">Pricing</a>
